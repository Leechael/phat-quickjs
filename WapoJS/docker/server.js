...
module.exports = (() => {
  ...
  // ../phat-quickjs/npm_package/wapo-env/dist/host.js
  var require_host = __commonJS({
    "../phat-quickjs/npm_package/wapo-env/dist/host.js"(exports) {
      "use strict";
      Object.defineProperty(exports, "__esModule", { value: true });
      exports.handle = exports.sendResponse = exports.WapoRequest = void 0;
      var WapoRequest = class extends Request {
        constructor(req, init) {
      ...
      exports.WapoRequest = WapoRequest;
      function sendResponse(response, req) {
  ...
  var HonoRequest = (_a = class {
    ...
    getAllDecodedParams() {
      const decoded = {};
      const keys4 = Object.keys(__privateGet(this, _matchResult)[0][this.routeIndex][1]);
  ...
  function buildMatcherFromPreprocessedRoutes(routes) {
    ...
    for (let i = 0, len = handlerData.length; i < len; i++) {
      for (let j = 0, len2 = handlerData[i].length; j < len2; j++) {
        ...
        const keys4 = Object.keys(map);
        for (let k = 0, len3 = keys4.length; k < len3; k++) {
          map[keys4[k]] = paramReplacementMap[map[keys4[k]]];
        }
      }
    }
    const handlerMap = [];
      ...
      if (/\*$/.test(path)) {
        ...
        if (method === METHOD_NAME_ALL) {
          Object.keys(middleware).forEach((m) => {
            var _a5;
            (_a5 = middleware[m])[path] || (_a5[path] = findMiddleware(middleware[m], path) || findMiddleware(middleware[METHOD_NAME_ALL], path) || []);
          });
        } else {
          (_a4 = middleware[method])[path] || (_a4[path] = findMiddleware(middleware[method], path) || findMiddleware(middleware[METHOD_NAME_ALL], path) || []);
        }
        Object.keys(middleware).forEach((m) => {
  ...
  function _curry2(fn) {
    return function f2(a, b) {
      switch (arguments.length) {
        ...
        default:
          return _isPlaceholder(a) && _isPlaceholder(b) ? f2 : _isPlaceholder(a) ? _curry1(function(_a4) {
            return fn(_a4, b);
  ...
  function _curry3(fn) {
    return function f3(a, b, c) {
      switch (arguments.length) {
        ...
        case 2:
          return _isPlaceholder(a) && _isPlaceholder(b) ? f3 : _isPlaceholder(a) ? _curry2(function(_a4, _c) {
            return fn(_a4, b, _c);
        ...
        default:
          return _isPlaceholder(a) && _isPlaceholder(b) && _isPlaceholder(c) ? f3 : _isPlaceholder(a) && _isPlaceholder(b) ? _curry2(function(_a4, _b) {
            return fn(_a4, _b, c);
          }) : _isPlaceholder(a) && _isPlaceholder(c) ? _curry2(function(_a4, _c) {
            return fn(_a4, b, _c);
          }) : _isPlaceholder(b) && _isPlaceholder(c) ? _curry2(function(_b, _c) {
            return fn(a, _b, _c);
          }) : _isPlaceholder(a) ? _curry1(function(_a4) {
            return fn(_a4, b, c);
          }) : _isPlaceholder(b) ? _curry1(function(_b) {
            return fn(a, _b, c);
  ...
  function _uniqContentEquals(aIterator, bIterator, stackA, stackB) {
    ...
    function eq(_a4, _b) {
      return _equals(_a4, _b, stackA.slice(), stackB.slice());
    }
  ...
  // src/index.ts
  var import_host = __toESM(require_host());

  // src/code_storage/ipfs.ts
  async function fetch_from_gateway(gateway, cid) {
    const resp = await fetch(`${gateway}/ipfs/${cid}`);
    const raw2 = await resp.text();
    return raw2;
  }
  var fetch_cloudflare = partial_default(fetch_from_gateway, [
    "https://cloudflare-ipfs.com"
  ]);
  var fetch_hardbin = partial_default(fetch_from_gateway, [
    "https://hardbin.com"
  ]);
  var fetch_dweb = partial_default(fetch_from_gateway, [
    "https://dweb.link"
  ]);
  var fetch_ipfs = partial_default(fetch_from_gateway, [
    "https://ipfs.io"
  ]);
  var fetch_4everland = partial_default(fetch_from_gateway, [
    "https://ipfs.4everland.io"
  ]);
  var fetch_pinata = partial_default(fetch_from_gateway, [
    "https://gateway.pinata.cloud"
  ]);
  var fetch_runfission = partial_default(fetch_from_gateway, [
    "https://runfission.com"
  ]);
  var fetch_trustless = partial_default(fetch_from_gateway, [
    "https://trustless-gateway.link"
  ]);
  var fetch_nftstorage = partial_default(fetch_from_gateway, [
    "https://nftstorage.link"
  ]);

  // src/sqld.ts
  async function sqld_execute(statements) {
    ...
    if (!is_default(Array, data)) {
      throw new Error(data.error);
    }
    return data;
  }

  // src/code_storage/cache_adapter.ts
  async function get_code(cid) {
    ...
    const obj = zipObj_default(col, row);
    return obj.code;
  }
  async function save_code(cid, code) {
    await sqld_execute([
      {
  ...
  // src/vault/sqld_adapter.ts
  var secretSchema = z.object({
    cid: z.string(),
    data: z.record(z.any()),
    inherit: z.string().optional()
  });
  var secretAccessTokenSchema = z.object({
    key: z.string(),
    token: z.string()
  });
  var vaultItemSchema = secretSchema.merge(secretAccessTokenSchema).extend({
  ...
  function inherit_vault_item(parents, data) {
    return mergeDeepWith_default(
      (l, r) => Array.isArray(l) ? concat_default(l, r) : r,
      parents,
      data
    );
  }
  async function get_vault_item(key) {
    const result = await sqld_execute([
      {
  ...
  async function get_vault_item_by_access_token(key, token) {
  ...
  async function run_guest_script(code, scriptId, path, req, requestId2) {
    let body = void 0;
...
Request ID: ${c.get("requestId")}`, 500);
  ...
  app.get("/logs/all/:scriptId{[a-zA-Z0-9/]+}", zValidator("query", logQuerySchema), async (c) => {
    ...
    const { requestId: requestId2, page, limit, format } = c.req.valid("query");
    const offset = (page - 1) * limit;
    console.log(scriptId, requestId2, page, limit, offset);
    let cols = [], rows = [];
    if (requestId2) {
      const result = await sqld_execute([{
        q: "SELECT * FROM logs WHERE script_id = ? AND request_id = ? ORDER BY created_at DESC, id DESC LIMIT ?, ?",
        params: [scriptId, requestId2, offset, limit]
      }]);
    ...
    const records = rows.map((row) => {
      const raw2 = zipObj_default(cols, row);
      const obj = omit_default(["id"], raw2);
      obj.created_at = new Date(raw2.created_at).toISOString();
      return obj;
    });
    if (format === "json") {
